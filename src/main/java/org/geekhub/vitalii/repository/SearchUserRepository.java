...
public class SearchUserRepository {
    ...
    public List<SearchUserDTO> findUser(String username) {
        String sql =
            "SELECT " +
                "c.username," +
                "COALESCE (COUNT(s.type) FILTER ( WHERE s.type = 1 ), 0) AS crypto," +
                "COALESCE (COUNT(s.type) FILTER ( WHERE s.type = 2 ), 0) AS currency," +
                "COALESCE (COUNT(s.type) FILTER ( WHERE s.type = 3 ), 0) AS share " +
            "FROM customer_stock " +
                "JOIN customer c on c.id = customer_stock.customer_id " +
                "JOIN stock s on s.symbol = customer_stock.stock_symbol " +
            "WHERE c.id IN (SELECT id FROM customer WHERE username = ?) AND c.role = 1 " +
            "GROUP BY c.username " +
            "UNION " +
            "SELECT " +
                "c.username," +
            ...
            "FROM customer_stock " +
                "JOIN customer c on c.id = customer_stock.customer_id " +
                "JOIN stock s on s.symbol = customer_stock.stock_symbol " +
            "WHERE NOT EXISTS(SELECT id FROM customer WHERE username = ?) AND c.role = 1 " +
            "GROUP BY c.username " +
